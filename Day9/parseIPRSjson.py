...
for result in iprscan['results']:
  ...
  for match in result['matches']:
    ...
    if 'accession' in match:
      ...
      if accession.startswith('PTHR'):
        ...
        for goXRef in match['goXRefs']:
          ...
          print(f"\t{goinfo}\t{goXRef['category']}")
    ...
    if 'entry' in match:
        entry_accession=match['entry']['accession']
        if entry_accession.startswith("IPR"):
            print(f"\tInterPro2GO name: {entry_accession['name']} description {entry_accession['description']}")
